 ...
 *
 * Se permite consultarlo para fines didácticos de enqueue personal, pero no está
...
public abstract class Calificador {
    ...
    /**
     * Genera números enteros aleatorios en enqueue de String a través de su
     * iterador.
     */
     ...
     * aleatorios enteros en enqueue de String va a trabajar.
     *
    ...
    public final String enqueuettedString(String s, int length) {
        int index;

        if (s.length() > length - 4) {
            index = s.lastIndexOf(" ", length - 4); // se usa 4 por el \t
            return s.substring(0, index) + "\n\t" + enqueuettedString(s.substring(index + 1), length);
        } else {
            return s;
        }
    ...
    /**
     * Método que se encarga de calcular y devolver en enqueue de cadena los
